...
export const SYMBOLS = [
  ...
  { key:'S', label:'🍬S', color:'bg-pink-300' }, // scatter only
];
...
export function generateBoard(w){
  ...
  for(let r=0;r<ROWS;r++){
    for(let c=0;c<COLS;c++){
      let p = weightedPick(w);
      if(p==='S'){
        const sc = b.filter(x=>x==='S').length;
        if(sc>=2) p = weightedPick({...w, S:0});
      }
      b[idx(r,c)] = p;
    }
  }
...
export function findClusters(board){
  ...
  for(let r=0;r<ROWS;r++){
    for(let c=0;c<COLS;c++){
      ...
      while(q.length){
        const [rr,cc] = q.shift(); comp.push(idx(rr,cc));
        for(const [dr,dc] of dirs){
          const nr=rr+dr,nc=cc+dc;
          if(!inBounds(nr,nc)) continue;
          const ni=idx(nr,nc);
          if(seen[ni]) continue;
          if(board[ni]===sym){ seen[ni]=true; q.push([nr,nc]); }
        }
      }
...
export function awardFreeSpins(sc){
  if(sc>=7) return 30; if(sc>=6) return 20; if(sc>=5) return 15; if(sc>=4) return 12; if(sc>=3) return 10;
  return 0;
}
